...
export function createConnectorRouter({
  ...
  secureCookies = false,
  mode = 'n8n',
  healthStatus = async () => ({ newestMeasurementAt: null, lastSuccessfulSync: null }),
  afterConnect = null,
  now = () => Date.now(),
}) {
  ...
  router.get(
    ...
    handler(async (_req, res) => {
      res.json({ ok: true, data: {
        ...(connector ? await connector.status() : { connected: false }),
        configured: Boolean(connector && oauth), mode, ...await healthStatus(),
      } });
    }),
  );

  router.use((_req, res, next) => {
    if (connector && oauth) return next();
    return res.status(503).json({ ok: false, message: 'Google connector is not configured' });
  });
  ...
  router.get(
    ...
    handler(async (req, res) => {
      ...
      try {
        ...
        return res.redirect('/settings?connected=1&recovery=pending');
      }
