  ...
  return {
    async recoverStaleClaims({ staleAfterMs = 15 * 60 * 1000 } = {}) {
      ...
      try {
        ...
        for (const { id } of recovered.rows) {
          await client.query('DELETE FROM sync_account_claims WHERE sync_chunk_id = $1', [id]);
        }
    ...
    async claimNextChunk(workerId) {
      ...
      try {
        ...
        const accountClaim = await client.query(
          ...
          [candidate.source_account_id, candidate.id],
        );
    ...
    async completeChunk(chunk, { nextPageToken }) {
      ...
      try {
        ...
        await client.query('DELETE FROM sync_account_claims WHERE sync_chunk_id = $1', [chunk.id]);
        if (nextPageToken) {
          await client.query(
            `INSERT INTO sync_chunks (
              id, sync_job_id, metric, operation, start_date, end_date_exclusive, page_token
