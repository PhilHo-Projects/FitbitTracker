...
export function createSyncRepository(pool, { advisoryLocks = true, now = () => Date.now() } = {}) {
  ...
  return {
      ...
      try {
        ...
        const job = (
          await client.query(
            `INSERT INTO sync_jobs (
              ...
              end_date_exclusive, requested_by, dedupe_key
            ) VALUES ($1, $2, $3, 'queued', $4, $5, $6, $7, $8, $9)
            ON CONFLICT (source_account_id, dedupe_key) DO NOTHING
            RETURNING *`,
            [
              id,
              sourceAccountId,
        ...
        if (!job || job.id !== id) {
          const existing = (await client.query(
            'SELECT * FROM sync_jobs WHERE source_account_id = $1 AND dedupe_key = $2',
            [sourceAccountId, dedupeKey],
          )).rows[0];
          const existingChunks = (await client.query('SELECT status FROM sync_chunks WHERE sync_job_id = $1', [existing.id])).rows;
          await client.query('COMMIT');
          return jobShape(existing, existingChunks);
        }
        ...
        if (isUniqueViolation(error)) {
          if (dedupeKey) {
            const recovery = (await client.query('SELECT * FROM sync_jobs WHERE source_account_id=$1 AND dedupe_key=$2', [sourceAccountId, dedupeKey])).rows[0];
            if (recovery) return jobShape(recovery, (await client.query('SELECT status FROM sync_chunks WHERE sync_job_id=$1', [recovery.id])).rows);
            throw Object.assign(new Error('Recovery waits for pending sync work to finish.'), { code: 'RECOVERY_WAITING_FOR_ACTIVE_JOB' });
          }
    ...
    async completeChunk(chunk, { nextPageToken, beforeCommit } = {}) {
      const client = await pool.connect();
      try {
        await client.query('BEGIN');
        const current = await client.query(
          `SELECT id FROM sync_chunks
        ...
        const otherStates = (
          await client.query(
            `SELECT status FROM sync_chunks WHERE sync_job_id = $1 AND id <> $2`,
            [chunk.sync_job_id, chunk.id],
          )
        ).rows;
        const pending = Boolean(nextPageToken) || otherStates.some(
          ({ status }) => status === 'queued' || status === 'running',
        );
        const jobStatus = pending
          ? null
          : otherStates.some(({ status }) => status === 'failed')
            ? 'completed_with_errors'
            : 'completed';
        if (jobStatus && beforeCommit) await beforeCommit({ client, jobStatus });
    ...
    async failChunk(chunk, error, { retryable = true, maxAttempts = 4, delayMs = 0, beforeCommit } = {}) {
      ...
      try {
        ...
        const failed = await client.query(
          `UPDATE sync_chunks
           SET status = $1, last_error = $2, claimed_by = NULL, claimed_at = NULL,
             available_at = $3, updated_at = CURRENT_TIMESTAMP, error_code = $6, http_status = $7
        ...
        if (!retry) {
          ...
          if (Number(remaining) === 0) {
            if (beforeCommit) await beforeCommit({ client, jobStatus: 'completed_with_errors' });
            await client.query(
              `UPDATE sync_jobs
    ...
    async jobStatus(jobId) {
      return (
        await pool.query('SELECT status FROM sync_jobs WHERE id = $1', [jobId])
      ).rows[0]?.status ?? null;
    },
    ...
    async status() {
      ...
      for (const job of jobs) {
        const chunks = (
          await pool.query('SELECT metric,status,error_code,http_status,completed_at FROM sync_chunks WHERE sync_job_id = $1', [job.id])
        ).rows;
        shaped.push({ ...jobShape(job, chunks), metricsStatus: [...new Set(chunks.map(c => c.metric))].map(metric => {
          const entries = chunks.filter(c => c.metric === metric);
          const failure = entries.find(c => c.status === 'failed');
          return { metric, status: failure ? 'failed' : entries.every(c => c.status === 'completed') ? 'completed' : 'pending',
            errorCode: failure?.error_code ?? null, httpStatus: failure?.http_status ?? null,
            completedChunks: entries.filter(c => c.status === 'completed').length, totalChunks: entries.length };
        }) });
      }
      return {
