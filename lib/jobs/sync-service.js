...
import { GOOGLE_HEALTH_METRICS, planSyncChunks, recentRange } from './planner.js';
...
export function createSyncService({
  ...
  const service = {
    ...
    start({ pollIntervalMs = 1000, syncIntervalMs = 3 * 60 * 60 * 1000 } = {}) {
      if (running) return;
      running = true;
      const poll = async () => {
        if (!running) return;
      ...
      repository
        ...
        .finally(() => {
          if (running) pollTimer = setTimeout(poll, 0);
        });
