...
export function createSyncService({
  ...
  async function ingest(chunk, response) {
    if (SLEEP_VITAL_METRICS.includes(chunk.metric)) return writer.upsertSleepVitals(chunk.source_account_id, chunk.metric, normalizeSleepVitals(chunk.metric, response.data));
    if (chunk.metric === 'oxygen-saturation') {
      return writer.upsertOxygenSaturationSamples(chunk.source_account_id, normalizeOxygenSaturationSamples(response.data));
    }
    if (chunk.metric === 'daily-oxygen-saturation') {
      return writer.upsertDailyOxygenSaturation(chunk.source_account_id, normalizeDailyOxygenSaturation(response.data));
    }
  ...
  const service = {
    ...
    async runOnce() {
      ...
      try {
        ...
        if (['oxygen-saturation', 'daily-oxygen-saturation', ...SLEEP_VITAL_METRICS].includes(chunk.metric) && error.code !== 'OXYGEN_CONTRACT_INVALID') {
          // Provider/DB messages can contain payloads; keep only classification.
          error = Object.assign(new Error(`${SLEEP_VITAL_METRICS.includes(chunk.metric) ? 'Sleep measurement' : 'Oxygen'} sync failed${Number.isInteger(error.status) ? ` (HTTP ${error.status})` : ''}`), {
            transient: error.transient, status: error.status,
          });
        }
