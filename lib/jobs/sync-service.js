...
export function createSyncService({
  ...
  function recalculateOnCompletion(chunk, completed = false) {
    return async ({ client, jobStatus }) => {
      ...
      if (!existingMetrics.length || existingMetrics.some(({ id, status }) => status !== 'completed' && !(completed && id === chunk.id))) return;
  ...
  const service = {
    async enqueue({
      ...
      if (selectedMetrics.includes('oxygen-saturation')) {
        const precedingDay = recentRange(start, 1).startDate;
        metricStartDates['oxygen-saturation'] = retentionStart && precedingDay < retentionStart ? retentionStart : precedingDay;
      }
    ...
    async runOnce() {
      ...
      try {
        ...
        if (['oxygen-saturation', 'daily-oxygen-saturation'].includes(chunk.metric) && error.code !== 'OXYGEN_CONTRACT_INVALID') {
          // Provider/DB messages can contain payloads; keep only classification.
          error = Object.assign(new Error(`Oxygen sync failed${Number.isInteger(error.status) ? ` (HTTP ${error.status})` : ''}`), {
