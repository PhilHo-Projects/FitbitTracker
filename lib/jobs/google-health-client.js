    ...
    async request(request) {
      ...
      try {
        ...
        if (!response.ok || body?.error) {
          if (response.status === 401) await connector.authorizationRejected?.(token);
          const error = new Error(`Google Health returned HTTP ${response.status}`);
          error.status = response.status;
          error.disconnected = response.status === 401;
          error.transient = response.status === 429 || response.status >= 500;
          throw error;
        }
        if (!body || typeof body !== 'object' || Array.isArray(body)) {
          throw Object.assign(new Error('Google Health returned an invalid response'), { transient: true, status: response.status, code: 'PROVIDER_CONTRACT_INVALID' });
        }
        return {
          ok: true,
