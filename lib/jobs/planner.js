import { SLEEP_VITALS, SLEEP_VITAL_METRICS } from '../metrics/sleep-vitals.js';
...
export const DEFAULT_SYNC_METRICS = GOOGLE_HEALTH_METRICS.filter(
  (metric) => metric !== 'total-calories',
);

export const RAW_SYNC_METRICS = [
  ...SLEEP_VITAL_METRICS.filter(metric => !SLEEP_VITALS[metric].daily),
  'oxygen-saturation',
  'heart-rate',
  'total-calories',
  'active-energy-burned',
...
const WINDOW_DAYS = {
  ...Object.fromEntries(SLEEP_VITAL_METRICS.map(metric => [metric, SLEEP_VITALS[metric].daily ? 90 : 14])),
  'oxygen-saturation': 14,
  'daily-oxygen-saturation': 90,
  'heart-rate': 14,
  'total-calories': 1,
  sleep: 90,
...
const OPERATIONS = {
  ...Object.fromEntries(SLEEP_VITAL_METRICS.map(metric => [metric, 'list'])),
...
export function planSyncChunks({
  ...
  return metrics
    .flatMap((metric) =>
      planMetricWindows({
        metric,
        startDate: metricStartDates[metric] ?? startDate,
        endDateExclusive,
      }),
    )
    .sort(
