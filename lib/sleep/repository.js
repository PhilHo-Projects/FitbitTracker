...
export function sleepAvailabilityState(fetch, count) {
  return fetch.lastAttemptStatus === 'failed' ? 'failed'
    : count > 0 ? fetch.fetchComplete ? 'available' : 'partial-observations'
      : fetch.fetchComplete ? 'empty-completed-fetch'
        : fetch.lastAttemptStatus === 'not-synced' ? 'never-fetched' : 'fetch-incomplete';
}
...
export function createSleepRepository(pool) {
  ...
  const repository = {
    ...
    availability: (accountId, date) => repository.availabilityRange(accountId, date, shiftDate(date, 1)),
    async availabilityRange(accountId, start, end) {
      const metrics = [
        "sleep",
      ...
      const rows = (
        await pool.query(
          ...
          [accountId, shiftDate(start, -1), end],
        )
      ).rows;
      const tables = { sleep: 'sleep_sessions', 'heart-rate': 'heart_rate_samples',
        'daily-resting-heart-rate': 'heart_rate_daily_summaries',
        'oxygen-saturation': 'oxygen_saturation_samples', 'daily-oxygen-saturation': 'oxygen_saturation_daily_summaries',
        ...Object.fromEntries(Object.entries(SLEEP_VITALS).map(([metric, spec]) => [metric, spec.table])) };
      const counts = Object.fromEntries(await Promise.all(Object.entries(tables).map(async ([metric, table]) => {
        const sample = ['heart-rate', 'oxygen-saturation', 'heart-rate-variability', 'respiratory-rate-sleep-summary'].includes(metric);
        const result = await pool.query(`SELECT COUNT(*) AS count FROM ${table} WHERE source_account_id=$1 AND civil_date >= $2 AND civil_date < $3${metric === 'daily-resting-heart-rate' ? ' AND resting_bpm IS NOT NULL AND resting_derived = false' : ''}`,
          [accountId, sample ? shiftDate(start, -1) : start, end]);
        return [metric, Number(result.rows[0].count)];
      })));
      return Object.fromEntries(
        metrics.map((metric) => {
          ...
          const fetch = oxygenFetchStatus(matches, {
            startDate: sample ? shiftDate(start, -1) : start,
          ...
          return [
            ...
            {
              ...fetch,
              recordCount: counts[metric],
              observationState: sleepAvailabilityState(fetch, counts[metric]),
              observationRange: { startDate: sample ? shiftDate(start, -1) : start, endDateExclusive: end },
              errorCode: fetch.errorCode === 'OXYGEN_SYNC_FAILED' ? 'SLEEP_METRIC_SYNC_FAILED' : fetch.errorCode,
              reconnectRequired: fetch.errorCode === 'GOOGLE_RECONNECT_REQUIRED',
              permissionDenied: fetch.errorCode === 'UPSTREAM_PERMISSION_DENIED' || fetch.httpStatus === 403,
