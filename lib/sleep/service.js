import { createSleepRepository, num, sleepAvailabilityState } from "./repository.js";
import {
  assessNight,
  sessionMetrics,
  sleepingHeart,
...
import { buildSleepInsights } from './insights.js';
import { createSleepSummary } from './summary.js';
...
export function createSleepService({ pool }) {
  ...
  async function insights({ date, days = 30, sources = {}, checkIns = null, includeSleepCheckIns = true } = {}) {
    sleepDate(date);
    if (![7, 30, 90].includes(days)) throw badRequest('Comparison days must be 7, 30, or 90');
    validateSleepSources(sources);
    const account = await repository.account();
    const end = shiftDate(date, 1), start = shiftDate(end, -days), historyStart = shiftDate(start, -days - 28);
    const [loaded, preferences, entries, availability, health, connection] = await Promise.all([
      account ? load(account, historyStart, end, sources) : { nights: Array.from({ length: days * 2 + 28 }, (_, i) => ({ date: shiftDate(historyStart, i), metrics: {}, sessions: [] })) },
      account ? repository.preferences(account.id) : { goalMinutes: 420 },
      includeSleepCheckIns && checkIns ? checkIns.list(start, end) : [],
      account ? repository.availabilityRange(account.id, start, end) : {},
      createHealthRepository(pool).getConnectionHealth(),
      pool.query("SELECT status FROM connector_credentials WHERE provider='google-health'"),
    ]);
    return buildSleepInsights({ date, days, sources, nights: loaded.nights, goalMinutes: preferences.goalMinutes,
      timezone: account?.timezone ?? null, checkIns: entries, includeSleepCheckIns, checkInsAvailable: !!checkIns,
      availability, freshness: { ...health, connectionState: connection.rows[0]?.status ?? 'unavailable' } });
  }
  return {
    repository,
    getPreferences,
    insights,
    async summary(options = {}) { return createSleepSummary(await insights({ ...options, includeSleepCheckIns: options.includeSleepCheckIns === true })); },
    ...
    async report({ date, sessionId, sources = {}, includeTracks = true } = {}) {
      ...
      for (const [key, value] of Object.entries(availability)) {
        if (observations[key] != null) value.recordCount = observations[key];
        value.observationState = sleepAvailabilityState(value, value.recordCount);
      }
