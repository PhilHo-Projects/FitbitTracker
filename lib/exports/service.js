...
const METRICS = ['sleep', 'heart', 'calories', 'oxygen'];
...
function shape(row) {
  ...
  return {
    ...
    includeSleepCheckIns: Boolean(row.include_sleep_check_ins),
...
function validate(input) {
  ...
  return {
    ...
    includeSleepCheckIns: input.includeSleepCheckIns === true,
...
export function createExportService({
  ...
  async function generate(job) {
    ...
    const dataset = await datasetService.buildAnalysisDataset(
      ...
      job.detailLevel,
      job.includeJournal,
      { loadRawRecords: job.detailLevel !== 'full', includeSleepCheckIns: job.includeSleepCheckIns },
    );
    ...
    if (job.metrics.includes('sleep')) {
      if (dataset.sleepInsights) {
        inventory.push(inventoryEntry('sleep-insights.json', 'json', 'The same versioned period and check-in comparisons used by the Sleep dashboard.'),
          inventoryEntry('sleep-report.md', 'markdown', 'Concise portable sleep report; optional ratings and tags follow the explicit check-in preference. Notes and journal text are excluded.'));
        entries.push({ name: 'sleep-insights.json', data: `${JSON.stringify(dataset.sleepInsights, null, 2)}\n` },
          { name: 'sleep-report.md', data: dataset.sleepReportMarkdown });
      }
      inventory.push(
        inventoryEntry(
          'sleep-sessions.csv',
    ...
    if (job.metrics.includes('oxygen')) {
      inventory.push(inventoryEntry('oxygen-saturation-daily.csv', 'csv', 'All provider daily records; bounds are confidence bounds.', dataset.oxygenDailySummaries.length));
      entries.push({ name: 'oxygen-saturation-daily.csv', data: toCsv(dataset.oxygenDailySummaries, EXPORT_COLUMNS['oxygen-saturation-daily.csv']) });
    }
    if (job.metrics.includes('oxygen') || job.metrics.includes('sleep') || job.includeSleepCheckIns) {
      inventory.push(inventoryEntry('analysis.json', 'json', 'Structured analysis, versioned sleep evidence, and normalized overnight measurements.'));
      entries.push({name:'analysis.json',data:`${JSON.stringify(dataset,null,2)}\n`});
    }
    if (job.includeSleepCheckIns) {
      inventory.push(inventoryEntry('sleep-check-ins.json','json','Decrypted optional check-ins, included only by explicit request.',dataset.sleepCheckIns.length));
      entries.push({name:'sleep-check-ins.json',data:`${JSON.stringify(dataset.sleepCheckIns,null,2)}\n`});
    }
    if (job.detailLevel === 'full') {
      if (job.metrics.includes('sleep')) {
        inventory.push(inventoryEntry('sleep-hrv-samples.jsonl','jsonl','All local intraday HRV records; source JSON and original timestamps retained.'));
        const samples=await datasetService.streamSleepHrvSamples(range);
        entries.push({name:'sleep-hrv-samples.jsonl',data:Readable.from((async function*(){for await(const row of samples)yield `${JSON.stringify(row)}\n`;})())});
      }
      const counts = await datasetService.rawCounts(range, job.metrics);
      if (job.metrics.includes('oxygen')) {
        inventory.push(inventoryEntry('oxygen-saturation-samples.csv', 'csv', 'All local oxygen provider records, original timestamps, offsets, and sources.', counts.oxygenSaturationSamples));
        entries.push({ name: 'oxygen-saturation-samples.csv', data: Readable.from(toCsvStream(await datasetService.streamOxygenSaturationSamples(range), EXPORT_COLUMNS['oxygen-saturation-samples.csv'])) });
      }
    ...
    const manifest = {
      ...
      sleepCheckInsIncluded: job.includeSleepCheckIns,
      sleepAnalysis: dataset.sleepAnalysis ? { analysisVersion:dataset.sleepAnalysis.analysisVersion, baselineRule:dataset.sleepAnalysis.baselineRule, preferences:dataset.sleepAnalysis.preferences, derivationRules:dataset.sleepAnalysis.derivationRules } : undefined,
      derivationFlags: dataset.derivationFlags,
      coverageWarnings: dataset.coverageWarnings,
      rawCoverage,
      oxygenSemantics: job.metrics.includes('oxygen') ? {
        dailyDate: 'Provider daily summary date; bounds are confidence bounds, not observed extrema.',
        rawDate: 'Recorded sample civil date in [start, end). Include the preceding date for the evening visible in a night chart.',
        sourceSelection: 'All provider sources and records retained. Joined daily values are null when multiple daily records exist.',
        standardDeviation: 'Provider historical deviation across prior daily averages (7–30 days), not nightly variation.',
        coldArchiveSupported: false,
      } : undefined,
    ...
    async create(input) {
      ...
      const result = await pool.query(
        `INSERT INTO export_jobs (
          ...
          metrics, detail_level, include_journal, include_png, created_at, include_sleep_check_ins
