...
const percent = value => Number.isFinite(value) ? `${value.toFixed(1)}%` : 'No data';
...
function dailyRows(days, oxygen = false) {
  ...
  return visible
    .map((day, index) => {
      ...
      return `
        ...
        <text x="360" y="${y}" class="row-value">${xml(duration(day.sleepAsleepMinutes))}</text>
        <text x="${oxygen ? 650 : 760}" y="${y}" class="row-value">${xml(number(day.heartRestingBpm, ' bpm'))}</text>
        <text x="${oxygen ? 960 : 1160}" y="${y}" class="row-value">${xml(number(day.calorieTotalKcal, ' kcal'))}</text>
        ${oxygen ? `<text x="1290" y="${y}" class="row-value">${xml(day.oxygenSummaryState === 'ambiguous' ? 'Ambiguous' : percent(day.oxygenAveragePercentage))}</text>` : ''}
      `;
    })
    .join('');
}
...
export function buildSummarySvg(dataset) {
  ...
  const sleepAverage = average(days.map(({ sleepAsleepMinutes }) => sleepAsleepMinutes));
  ...
  const oxygen = dataset.metrics?.includes('oxygen');
  const oxygenAverage = average(days.map(day => day.oxygenAveragePercentage));
  const rangeLabel =
  ...
  return `
    <svg width="1600" height="1000" viewBox="0 0 1600 1000" xmlns="http://www.w3.org/2000/svg">
      <style>
        ...
        .metric-label { font-size: ${oxygen ? 18 : 20}px; fill: #9aa6b8; }
        .metric-value { font-size: ${oxygen ? 42 : 48}px; font-weight: 700; }
        .metric-note { font-size: ${oxygen ? 16 : 18}px; fill: #7f8b9e; }
      ...
      <text x="80" y="380" class="metric-note">${days.filter(({ sleepAsleepMinutes }) => Number.isFinite(sleepAsleepMinutes)).length} days measured</text>

      <text x="${oxygen ? 440 : 585}" y="282" class="metric-label">Average resting heart rate</text>
      <text x="${oxygen ? 440 : 585}" y="344" class="metric-value">${xml(number(restingAverage, ' bpm'))}</text>
      <text x="${oxygen ? 440 : 585}" y="380" class="metric-note">${oxygen ? 'From stored daily values' : 'No smoothed or inferred chart values'}</text>

      <text x="${oxygen ? 800 : 1090}" y="282" class="metric-label">Average total expenditure</text>
      <text x="${oxygen ? 800 : 1090}" y="344" class="metric-value">${xml(number(calorieAverage, ' kcal'))}</text>
      <text x="${oxygen ? 800 : 1090}" y="380" class="metric-note">${oxygen ? 'Energy burned; not food intake' : 'Active and basal energy, never food intake'}</text>
      ${oxygen ? `<text x="1160" y="282" class="metric-label">Blood oxygen · daily average</text><text x="1160" y="344" class="metric-value">${percent(oxygenAverage)}</text><text x="1160" y="380" class="metric-note">${days.filter(day => Number.isFinite(day.oxygenAveragePercentage)).length} days measured · equal weight</text>` : ''}
      ...
      <text x="${oxygen ? 650 : 760}" y="650" class="column">RESTING HEART</text>
      <text x="${oxygen ? 960 : 1160}" y="650" class="column">TOTAL CALORIES</text>
      ${oxygen ? '<text x="1290" y="650" class="column">SpO₂ AVERAGE</text>' : ''}
      ${dailyRows(days, oxygen)}
