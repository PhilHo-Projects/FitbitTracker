...
export function createAnalysisDatasetService({ pool, journalRepository = null, batchSize = 2_000 }) {
  ...
  async function* pagedRows(sql, params, mapper) {
    let offset = 0;
    while (true) {
      const result = await pool.query(`${sql} LIMIT $${params.length + 1} OFFSET $${params.length + 2}`, [
        ...params,
        batchSize,
        offset,
      ]);
  ...
  async function* calorieIntervalRows(range, sourceAccountId) {
    yield* pagedRows(
      `SELECT *
       ...
       WHERE source_account_id = $1 AND civil_date >= $2 AND civil_date < $3
       ORDER BY start_time, metric_type, id`,
      [sourceAccountId, range.startDate, range.endDateExclusive],
      mapCalorieInterval,
    );
  }
