  ...
  return {
    ...
    async status() {
      ...
      return {
        ...
        recoveryFrom: row?.recoveryFrom ?? null,
        recoveryPending: row?.recoveryPending ?? false,
      };
    },
    ...
    async connectWithCode(code) {
      ...
      const previous = await repository.load(PROVIDER);
      const recoveryDates = [previous?.disconnectedAt, previous?.recoveryPending ? previous.recoveryFrom : null]
      ...
      await repository.save(PROVIDER, {
        ...
        recoveryFrom: recoveryDates[0] ?? null,
      });
    },

    disconnect: () => repository.markDisconnected(PROVIDER, 'Disconnected by the owner'),
    authorizationRejected: token => repository.withLock(PROVIDER, async (row, client) => {
      // A delayed response for an old token must not disconnect a newly consented account.
      if (row?.status === 'connected' && row.accessToken === token)
        await repository.markDisconnected(PROVIDER, 'Google authorization expired. Reconnect Google Health.', client ?? undefined);
    }),
    recoveryScheduled: (generation) => repository.recoveryScheduled(PROVIDER, generation),
  };
}
