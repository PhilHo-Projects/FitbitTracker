...
import { decodeArchiveNullableText, readCsvRows } from './csv.js';

const CATALOG_TIMESTAMP_COLUMNS = [
...
function canonicalCivilDate(value) {
  if (typeof value !== 'string' || !/^\d{4}-\d{2}-\d{2}$/.test(value)) {
    throw new Error('Archive month must be canonical PostgreSQL date text');
  }
  return value;
}

function isMemoryDatabase(database) {
  return database.constructor?.name === 'MemPg';
}

function catalogProjection(database) {
  if (isMemoryDatabase(database)) return '*';
  return [
    'id',
    'source_account_id',
    'archive_month::text AS archive_month',
    'archive_version',
    'is_active',
    'state',
    'object_key',
    'heart_sample_count',
    'calorie_interval_count',
    ...CATALOG_TIMESTAMP_COLUMNS.slice(0, 2).map((column) =>
    ...
    'error_code',
    'error_message',
    ...CATALOG_TIMESTAMP_COLUMNS.slice(2).map((column) =>
      `to_char(${column} AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS.US"Z"') AS ${column}`),
  ].join(',\n           ');
}
...
function encodeListCursor(row) {
  return Buffer.from(JSON.stringify({
    month: canonicalCivilDate(row.archive_month),
...
function archiveCell(value, column, nullableTextEncoding) {
  if (column === 'upstream_sample_id') {
    return decodeArchiveNullableText(value, nullableTextEncoding);
  }
  return column === 'utc_offset_seconds' && value === '\\N' ? null : value;
}
...
async function loadArchiveCsvStage({
  ...
  for await (const row of readCsvRows(filePath)) {
    ...
    rows.push(row.map((value, index) =>
      archiveCell(value, columns[index], nullableTextEncoding)));
    count += 1;
...
export function createHealthArchiveRepository(database, { advisoryLocks = true } = {}) {
  ...
  async function getById(id, queryable = database) {
    const result = await queryable.query(
      `SELECT ${catalogProjection(queryable)}
       FROM health_archive_catalog WHERE id = $1`,
      [id],
    );
  ...
  async function reserveMonth(sourceAccountId, archiveMonth, { rebuild = false } = {}) {
    return withTransaction(database, async (client) => {
      let active = (await client.query(
        `SELECT ${catalogProjection(client)} FROM health_archive_catalog
         WHERE source_account_id = $1 AND archive_month = $2::date AND is_active = true
         FOR UPDATE`,
        [sourceAccountId, archiveMonth],
      )).rows[0];
      if (rebuild && !active) {
        throw new Error('Explicit archive rebuild requires a verified prune-mismatch catalog entry');
      }
      if (active && rebuild && (
        active.state !== 'verified'
        || active.error_code !== 'ARCHIVE_PRUNE_FAILED'
      )) {
        throw new Error('Explicit archive rebuild requires a verified prune-mismatch catalog entry');
      }
      if (active && !rebuild && active.state !== 'failed') return active;
      if (active) {
        await client.query(
      ...
      active = (await client.query(
        `INSERT INTO health_archive_catalog (
         ...
         RETURNING ${catalogProjection(client)}`,
  ...
  return {
    async withMonthLock(sourceAccountId, archiveMonth, operation) {
      const ownsClient = typeof database.connect === 'function' && typeof database.release !== 'function';
      const client = ownsClient ? await database.connect() : database;
      const lockKey = `${sourceAccountId}:${canonicalCivilDate(archiveMonth)}`;
    ...
    async list({ sourceAccountId, archiveMonth, state, limit = 50, cursor } = {}) {
      ...
      const rows = (await database.query(
        `SELECT ${catalogProjection(database)} FROM health_archive_catalog
    ...
    async listEligibleMonths({ today = new Date().toISOString().slice(0, 10), retentionDays = 90 } = {}) {
      return (await database.query(
        `WITH months AS (
         ...
         SELECT months.source_account_id,
                ${isMemoryDatabase(database) ? 'months.archive_month' : 'months.archive_month::text'} AS archive_month
         FROM months
    ...
    markBuilding(id) {
      return updateAndReturn(
        `UPDATE health_archive_catalog
         ...
         RETURNING ${catalogProjection(database)}`,
        [id],
      );
    },
    ...
    async recordVerificationFailure(id, failure) {
      ...
      return updateAndReturn(
        `UPDATE health_archive_catalog
         SET state = CASE
               WHEN state IN ('pruning', 'pruned', 'superseded') THEN state ELSE 'failed'
             END,
    ...
    async recordVerificationSuccess(id) {
      return updateAndReturn(
        `UPDATE health_archive_catalog
         SET state = CASE
               WHEN state IN ('pruning', 'pruned', 'superseded') THEN state
               WHEN is_active = true THEN 'verified'
               ELSE state
             END,
             verified_at = COALESCE(verified_at, CURRENT_TIMESTAMP),
             error_code = CASE
               WHEN error_code = 'ARCHIVE_VERIFY_FAILED' THEN NULL ELSE error_code
             END,
             error_message = CASE
               WHEN error_code = 'ARCHIVE_VERIFY_FAILED' THEN NULL ELSE error_message
             END,
             updated_at = CURRENT_TIMESTAMP
         WHERE id = $1
           ...
           AND (
             state IN ('pruning', 'pruned', 'superseded')
             OR (is_active = true AND state IN ('building', 'uploaded', 'verified', 'failed'))
           )
         RETURNING ${catalogProjection(database)}`,
        [id],
      );
