  ...
  try {
    ...
    const heartSamples = await importCsv({
      ...
      mapRow: (row) => [
        ...
        row[1],
        row[2],
        nullableNumber(row[3]) === null ? null : numeric(row[3], 'UTC offset'),
        numeric(row[4], 'heart rate'),
        decodeArchiveNullableText(row[5], manifest.nullableTextEncoding),
      ],
    });
    const calorieIntervals = await importCsv({
      ...
      mapRow: (row) => [
        ...
        row[3],
        row[4],
        nullableNumber(row[5]) === null ? null : numeric(row[5], 'UTC offset'),
        numeric(row[6], 'calories'),
        decodeArchiveNullableText(row[7], manifest.nullableTextEncoding),
      ],
    });
