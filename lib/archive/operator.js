...
function positiveListLimit(value) {
  const parsed = Number(value);
...
export function parseHealthArchiveArgs(args) {
  ...
  if (command === 'list') result.limit = 50;
  if (command === 'run') Object.assign(result, { execute: false, prune: false, rebuild: false });
  ...
  for (let index = 1; index < args.length;) {
    ...
    if (argument === '--rebuild' && command === 'run') {
      result.rebuild = true;
      index += 1;
      continue;
    }
    ...
    for (const [name, property, commands] of definitions) {
      ...
      if (!parsed) continue;
      result[property] = property === 'batchSize'
        ? positiveBatchSize(parsed.value)
        : property === 'limit'
          ? positiveListLimit(parsed.value)
          : parsed.value;
  ...
  if (command === 'run' && result.rebuild && !result.execute) {
    throw new Error('--rebuild requires --execute');
  }
  if (command === 'run' && result.rebuild && result.prune) {
    throw new Error('--rebuild and --prune cannot be combined');
  }
