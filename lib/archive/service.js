...
async function verifyDefaultStoredArchive({
  ...
  try {
    ...
    if (
      decrypted.header.sourceAccountId !== catalog.source_account_id
      || decrypted.header.archiveMonth !== isoCivilDate(catalog.archive_month)
      || decrypted.header.encryptionKeyVersion !== Number(catalog.encryption_key_version)
...
export function createHealthArchiveService({
  ...
  async function archiveMonth({ sourceAccountId, archiveMonth, dryRun = false, prune = false }) {
    ...
    return repository.withMonthLock(sourceAccountId, archiveMonth, async (lockedRepository) => {
      ...
      if (catalog.state === 'pruned') return { catalog, idempotent: true };
      ...
      try {
        ...
        if (prune && catalog.state !== 'pruned') {
          stage = 'prune';
          const removed = await lockedRepository.pruneVerifiedMonth(catalog.id, { batchSize });
          catalog = { ...catalog, state: 'pruned' };
          return { catalog, idempotent: false, removed };
        }
      ...
      return repository.withMonthLock(
        ...
        async (lockedRepository) => {
          ...
          try {
            ...
            await lockedRepository.recordFailure(id, failure);
