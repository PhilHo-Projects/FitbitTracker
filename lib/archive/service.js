...
function canonicalArchiveDate(value) {
  if (typeof value !== 'string' || !/^\d{4}-\d{2}-\d{2}$/.test(value)) {
    throw new Error('Archive catalog date was not returned as canonical text');
  }
  return value;
}

function clockCivilDate(value) {
...
function civilDayNumber(value) {
  ...
  if (month < 1 || month > 12 || day < 1 || day > daysInMonth[month - 1]) {
    throw new Error('Archive civil date is invalid');
  }
  return Math.floor(Date.UTC(year, month - 1, day) / (24 * 60 * 60 * 1000));
}

function isRecoverableCatalog(catalog) {
  if (['verified', 'pruned'].includes(catalog.state)) return true;
  if (catalog.state !== 'superseded' || catalog.is_active !== false || !catalog.verified_at) {
    return false;
  }
  return typeof catalog.object_key === 'string'
    && catalog.object_key.length > 0
    && /^[a-f0-9]{64}$/.test(catalog.ciphertext_hash)
    && /^[a-f0-9]{64}$/.test(catalog.plaintext_hash)
    && catalog.byte_size !== null
    && catalog.byte_size !== undefined
    && Number.isFinite(Number(catalog.byte_size))
    && Number(catalog.byte_size) >= 0
    && Number.isInteger(Number(catalog.encryption_key_version))
    && Number(catalog.encryption_key_version) > 0;
}

export function assertCatalogEnvelopeBinding(catalog, header) {
  if (
    header?.sourceAccountId !== catalog.source_account_id
    || header?.archiveMonth !== canonicalArchiveDate(catalog.archive_month)
...
export function isArchiveMonthEligible(archiveMonth, now = new Date(), retentionDays = 90) {
  const normalized = canonicalArchiveDate(archiveMonth);
  if (!/^\d{4}-\d{2}-01$/.test(normalized)) {
    throw new Error('Archive month must be the first civil date of a month');
  }
  const year = Number(normalized.slice(0, 4));
  const month = Number(normalized.slice(5, 7));
  if (month < 1 || month > 12) throw new Error('Archive month is invalid');
  const nextMonthDay = Math.floor(Date.UTC(year, month, 1) / (24 * 60 * 60 * 1000));
  const finalCivilDay = nextMonthDay - 1;
  return civilDayNumber(clockCivilDate(now)) - finalCivilDay >= retentionDays;
}
...
async function verifyDefaultStoredArchive({
  ...
  try {
    ...
    const validated = await extractMonthBundle({
      ...
      expectedArchiveMonth: canonicalArchiveDate(catalog.archive_month),
...
export function createHealthArchiveService({
  ...
  async function archiveMonth({
    ...
    if (prune && rebuild) throw new Error('Archive rebuild and prune cannot be combined');
    ...
    return repository.withMonthLock(sourceAccountId, archiveMonth, async (lockedRepository) => {
      let catalog = await lockedRepository.reserveMonth(
        sourceAccountId,
        archiveMonth,
        { rebuild },
      );
      ...
      try {
        ...
        if (prune) {
          ...
          await verify({
            catalog,
            async withValidatedArchive({ directory, manifest }) {
              ...
              removed = await lockedRepository.pruneVerifiedMonth(catalog.id, {
                ...
                nullableTextEncoding: manifest?.nullableTextEncoding,
              });
  ...
  return {
    ...
    async verifyById(id) {
      ...
      return repository.withMonthLock(
        ...
        canonicalArchiveDate(initial.archive_month),
        async (lockedRepository) => {
          ...
          catalog = await lockedRepository.recordVerificationSuccess(id);
    ...
    async extractById({ id, outputDirectory }) {
      ...
      if (!isRecoverableCatalog(catalog)) {
    ...
    async importById({ id, targetPool, importBatchSize = batchSize }) {
      ...
      try {
        ...
        return await importExtractedMonth({
          directory: extractedDirectory,
          targetPool,
