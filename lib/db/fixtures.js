...
import { parseOxygenTime } from '../metrics/oxygen-time.js';
...
export async function seedFixtures(pool, { anchorDate = '2026-07-16' } = {}) {
  ...
  try {
    ...
    for (let dayIndex = 0; dayIndex < 8; dayIndex += 1) {
      ...
      const sourceMetadata = { device: { displayName: 'Synthetic preview watch' } };
      const sourceKey = hashSourceMetadata(sourceMetadata);
      const oxygenWriter = createOxygenWriter(client, { clientOwnedByCaller: true });
      if (dayIndex !== 2) await oxygenWriter.upsertDailyOxygenSaturation(ACCOUNT_ID, [{
        providerKey: `fixture-daily-oxygen-${civilDate}`, providerId: `fixture-daily-oxygen-${civilDate}`, sourceKey, sourceMetadata,
        civilDate, averagePercentage: 96.4 - dayIndex * .15, lowerBoundPercentage: 94.2 - dayIndex * .1,
        upperBoundPercentage: 98.1, standardDeviationPercentage: .6, qualityFlags: [], sourceFields: { fixture: true },
      }]);
      if (dayIndex !== 3) await oxygenWriter.upsertOxygenSaturationSamples(ACCOUNT_ID, Array.from({ length: Math.floor(durationSeconds / 60) }, (_, minute) => {
        const sampledAt = new Date(startMs + minute * 60_000).toISOString();
        const { epochNanoseconds, ...time } = parseOxygenTime({ physicalTime: sampledAt, utcOffset: '-14400s' });
        return { providerKey: `fixture-oxygen-${civilDate}-${minute}`, providerId: `fixture-oxygen-${civilDate}-${minute}`,
          sourceKey, sourceMetadata, ...time, percentage: minute === 182 ? 92.3 : 96.4 + Math.sin(minute / 14) * .9,
          sourceFields: { fixture: true } };
      }).filter((_, minute) => minute < 90 || minute > 98));
