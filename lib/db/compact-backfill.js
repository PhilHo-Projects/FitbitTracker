...
const legacyMetrics = {
  heart: {
    ...
    time: 'sampled_at',
    dataType: "'heart-rate'",
    sql: `SELECT id, provider_id, civil_date::text AS civil_date,
            to_char(sampled_at AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS.US"Z"') AS sampled_at, utc_offset_seconds,
  ...
  calories: {
    table: 'calorie_intervals',
    time: 'start_time',
    dataType: "CASE metric_type WHEN 'total' THEN 'total-calories' WHEN 'active' THEN 'active-energy-burned' ELSE 'basal-energy-burned' END",
    sql: `SELECT id, provider_id, civil_date::text AS civil_date, metric_type,
...
async function preflightIdentities(pool, sourceAccountId, metric) {
  const { table, time, dataType } = legacyMetrics[metric];
  // JSONB equality canonicalizes object keys just like sourceMetadataForRecord.
  // GROUP BY can spill under PostgreSQL's work_mem limit; no identity set grows in Node.
  // Compare native timestamptz values to retain all six microsecond digits.
  const result = await pool.query(`SELECT 1 AS duplicate FROM ${table}
    WHERE source_account_id = $1
    GROUP BY ${time}, ${metric === 'calories' ? 'metric_type,' : ''}
      CASE WHEN jsonb_typeof(source_fields->'dataSource') IN ('object', 'array')
        THEN jsonb_build_object('dataType', ${dataType}, 'dataSource', source_fields->'dataSource')
        WHEN jsonb_typeof(device) IN ('object', 'array') AND device NOT IN ('{}'::jsonb, '[]'::jsonb)
        THEN jsonb_build_object('dataType', ${dataType}, 'device', device)
        ELSE jsonb_build_object('dataType', ${dataType}) END
    HAVING COUNT(*) > 1 LIMIT 1`, [sourceAccountId]);
  if (result.rows.length) throw new Error(`Duplicate ${metric === 'heart' ? 'heart' : 'calorie'} semantic identity`);
}

async function finalizeDates(pool, sourceAccountId, batchSize, metricWriter) {
  let cursor = null;
  let count = 0;
  for (;;) {
    const result = await pool.query(`SELECT civil_date::text AS civil_date FROM (
      SELECT civil_date FROM heart_rate_samples WHERE source_account_id = $1 AND ($2::date IS NULL OR civil_date > $2::date)
      UNION
      SELECT civil_date FROM calorie_intervals WHERE source_account_id = $1 AND ($2::date IS NULL OR civil_date > $2::date)
    ) dates ORDER BY civil_date LIMIT $3`, [sourceAccountId, cursor, batchSize]);
    if (!result.rows.length) return count;
    for (const row of result.rows) {
      await metricWriter.recalculateDaily(sourceAccountId, row.civil_date);
      count += 1;
    }
    cursor = result.rows.at(-1).civil_date;
  }
}
...
export async function runCompactHealthOperation({
  ...
  for (const { id: sourceAccountId } of accounts) {
    for (const metric of ['heart', 'calories']) {
      await preflightIdentities(pool, sourceAccountId, metric);
  ...
  for (const { id: sourceAccountId } of accounts) {
    ...
    result.finalizedDates += await finalizeDates(pool, sourceAccountId, boundedBatchSize, metricWriter);
  }
