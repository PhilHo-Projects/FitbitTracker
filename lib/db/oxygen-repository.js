...
export function oxygenFetchStatus(rows, { startDate, endDateExclusive }) {
  ...
  for (const window of windows.values()) {
    ...
    const failure = window.rows.find(row => row.status === 'failed');
    if (!latest || at > latest.at || (at === latest.at && status === 'failed')) latest = { at, status,
  ...
  return { lastAttemptStatus: latest?.status ?? 'not-synced', lastAttemptAt: latest?.at ?? null,
    ...
    errorCode: latest?.status === 'failed' ? latest.errorCode ?? 'OXYGEN_SYNC_FAILED' : null,
