...
export function createHealthRepository(pool, availabilityOptions = {}) {
  ...
  async function getConnectionHealth(source) {
    ...
    if (!source) return { newestMeasurementAt: null, lastSuccessfulSync: null };
    const [newest, sync] = await Promise.all([
      pool.query(
      ...
      pool.query(`SELECT finished_at FROM sync_jobs WHERE status = 'completed' ORDER BY finished_at DESC LIMIT 1`),
    ]);
