...
import { validateMetricRange } from '../metrics/range.js';
import { createOxygenRepository, compactOxygenDay, validateOxygenRange } from './oxygen-repository.js';
...
async function heartBuckets(pool, sourceAccountId, startDate, endDateExclusive) {
  const params = [sourceAccountId, startDate, endDateExclusive];
  if (pool.constructor?.name !== 'MemPg') {
    const result = await pool.query(
      `SELECT date_bin(INTERVAL '5 minutes', sampled_at, TIMESTAMPTZ '1970-01-01T00:00:00Z') AS bucket_time,
              AVG(beats_per_minute) AS average_bpm, MIN(beats_per_minute) AS minimum_bpm,
              MAX(beats_per_minute) AS maximum_bpm, COUNT(*) AS sample_count
       FROM heart_rate_samples
       WHERE source_account_id = $1 AND civil_date >= $2 AND civil_date < $3
       GROUP BY bucket_time ORDER BY bucket_time`, params,
    );
    return result.rows.map(row => ({ time: iso(row.bucket_time), averageBpm: round(Number(row.average_bpm), 1),
      minimumBpm: Number(row.minimum_bpm), maximumBpm: Number(row.maximum_bpm), count: Number(row.sample_count) }));
  }
  // pg-mem does not implement date_bin. Fixture aggregation retains only running totals.
  const samples = await pool.query(`SELECT sampled_at, beats_per_minute FROM heart_rate_samples
    WHERE source_account_id = $1 AND civil_date >= $2 AND civil_date < $3 ORDER BY sampled_at`, params);
  const buckets = new Map();
  for (const sample of samples.rows) {
    const time = Math.floor(new Date(sample.sampled_at).getTime() / 300_000) * 300_000;
    const value = Number(sample.beats_per_minute);
    const bucket = buckets.get(time) ?? { sum: 0, min: Infinity, max: -Infinity, count: 0 };
    bucket.sum += value; bucket.count += 1;
    bucket.min = Math.min(bucket.min, value); bucket.max = Math.max(bucket.max, value);
    buckets.set(time, bucket);
  }
  return [...buckets].map(([time, bucket]) => ({ time: new Date(time).toISOString(),
    averageBpm: round(bucket.sum / bucket.count, 1), minimumBpm: bucket.min, maximumBpm: bucket.max, count: bucket.count }));
}

async function calorieBuckets(pool, sourceAccountId, startDate, endDateExclusive) {
  const params = [sourceAccountId, startDate, endDateExclusive];
  if (pool.constructor?.name !== 'MemPg') {
    const result = await pool.query(`SELECT start_time,
      SUM(CASE WHEN metric_type = 'active' THEN kilocalories ELSE 0 END) AS active_kcal,
      SUM(CASE WHEN metric_type = 'basal' THEN kilocalories ELSE 0 END) AS basal_kcal,
      SUM(CASE WHEN metric_type = 'total' THEN kilocalories END) AS total_kcal
      FROM calorie_intervals WHERE source_account_id = $1 AND civil_date >= $2 AND civil_date < $3
      GROUP BY start_time ORDER BY start_time`, params);
    return result.rows.map(row => ({ time: iso(row.start_time), activeKcal: round(Number(row.active_kcal)),
      basalKcal: round(Number(row.basal_kcal)), totalKcal: round(row.total_kcal === null
        ? Number(row.active_kcal) + Number(row.basal_kcal) : Number(row.total_kcal)) }));
  }
  const result = await pool.query(`SELECT metric_type, start_time, kilocalories FROM calorie_intervals
    WHERE source_account_id = $1 AND civil_date >= $2 AND civil_date < $3 ORDER BY start_time`, params);
  const buckets = new Map();
  for (const interval of result.rows) {
    const time = iso(interval.start_time);
    const bucket = buckets.get(time) ?? { time, activeKcal: 0, basalKcal: 0, totalKcal: null };
    ...
    if (interval.metric_type === 'total') bucket.totalKcal = (bucket.totalKcal ?? 0) + value;
    buckets.set(time, bucket);
  }
  return [...buckets.values()].map(bucket => ({ time: bucket.time,
    activeKcal: round(bucket.activeKcal), basalKcal: round(bucket.basalKcal),
...
export function createHealthRepository(pool, availabilityOptions = {}) {
  const oxygen = createOxygenRepository(pool);
  ...
  async function getSleep(sourceAccountId, date) {
    ...
    return {
      ...
      timeToSleepMinutes: Number(session.time_to_sleep_seconds) === 0 && session.source_fields?.normalizationVersion !== 2 ? null : minutes(session.time_to_sleep_seconds),
  ...
  async function getConnectionHealth(source) {
    source ??= await account();
    if (!source) return { newestMeasurementAt: null, lastSuccessfulSync: null, lastSuccessfulFetch: null };
    const [newest, sync, fetch] = await Promise.all([
      pool.query(
        `SELECT MAX(measured_at) AS newest_measurement_at FROM (
           SELECT MAX(sampled_at) AS measured_at FROM heart_rate_samples WHERE source_account_id = $1
           UNION ALL
           SELECT MAX(start_time) AS measured_at FROM sleep_sessions WHERE source_account_id = $1
           UNION ALL
           SELECT MAX(start_time) AS measured_at FROM calorie_intervals WHERE source_account_id = $1
           UNION ALL
           SELECT MAX(sampled_at) AS measured_at FROM oxygen_saturation_samples WHERE source_account_id = $1
           UNION ALL
           SELECT MAX(sampled_at) AS measured_at FROM sleep_hrv_samples WHERE source_account_id = $1
           UNION ALL
           SELECT MAX(sampled_at) AS measured_at FROM sleep_respiratory_summaries WHERE source_account_id = $1
         ) measurements`, [source.id]),
      pool.query(`SELECT finished_at FROM sync_jobs WHERE source_account_id = $1 AND status = 'completed' ORDER BY finished_at DESC LIMIT 1`, [source.id]),
      pool.query(`SELECT MAX(c.completed_at) AS fetched_at FROM sync_chunks c
        JOIN sync_jobs j ON j.id = c.sync_job_id WHERE j.source_account_id = $1
        AND c.status = 'completed' AND (c.next_page_token IS NULL OR c.next_page_token = '')`, [source.id]),
    ]);
    return { newestMeasurementAt: iso(newest.rows[0]?.newest_measurement_at), lastSuccessfulSync: iso(sync.rows[0]?.finished_at),
      lastSuccessfulFetch: iso(fetch.rows[0]?.fetched_at) };
  }

  return {
    async getOxygenRange(startDate, endDateExclusive, resolution = 'day', selection = {}) {
      validateOxygenRange(startDate, endDateExclusive, resolution, selection);
      const source = await account();
      return resolution === 'night' ? oxygen.getDay(source?.id ?? null, startDate, selection)
        : oxygen.getRange(source?.id ?? null, { startDate, endDateExclusive }, selection);
    },
    ...
    async getDashboard(date) {
      ...
      if (!source) {
        return {
          ...
          oxygenSaturation: compactOxygenDay(await oxygen.getDay(null, date)),
          journal: [],
          coverage: { sleep: 'missing', heart: 'missing', calories: 'missing', oxygen: 'not-synced' },
          sync: { lastSuccessfulSync: null, stale: true },
        };
      }

      const [sleep, heart, calories, daily, health, oxygenDay] = await Promise.all([
        ...
        getConnectionHealth(source),
        oxygen.getDay(source.id, date),
      ]);
      ...
      return {
        ...
        newestMeasurementAt: health.newestMeasurementAt,
        membershipStartDate: dateOnly(source.membership_start_date),
        sleep,
        heart,
        calories,
        oxygenSaturation: compactOxygenDay(oxygenDay),
        journal: [],
        coverage: { ...coverage, oxygen: oxygenDay.dataState },
    ...
    async getSleepRange(startDate, endDateExclusive) {
      validateMetricRange(startDate, endDateExclusive);
      const source = await account();
    ...
    async getHeartRange(startDate, endDateExclusive, resolution = 'day') {
      validateMetricRange(startDate, endDateExclusive, resolution);
      const source = await account();
      if (!source) {
        return {
          startDate,
      ...
      const points = await heartBuckets(pool, source.id, startDate, endDateExclusive);
    ...
    async getCaloriesRange(startDate, endDateExclusive, resolution = 'day') {
      ...
      const intervals = await calorieBuckets(pool, source.id, startDate, endDateExclusive);
      return {
        startDate,
        endDateExclusive,
        timezone: source.timezone,
