...
import { oxygenInstantNanoseconds as instantNanoseconds } from '../metrics/oxygen-time.js';
import {
...
export function createCompactMetricWriter(pool) {
  ...
  return {
    async upsertHeartSamples(sourceAccountId, samples) {
      ...
      assertUnique(
        rows,
        ({ sourceStreamId, sampledAt }) => `${sourceStreamId}:${instantNanoseconds(sampledAt)}`,
        'heart',
      );
    ...
    async upsertCalorieIntervals(sourceAccountId, intervals) {
      ...
      assertUnique(
        ...
        ({ sourceStreamId, metricType, startTime }) =>
          `${sourceStreamId}:${metricType}:${instantNanoseconds(startTime)}`,
        'calorie',
      );
