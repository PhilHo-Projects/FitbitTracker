...
async function acquireDatabase(database) {
  if (typeof database.connect === 'function') return database.connect();
  return {
...
export function createMetricWriter(
  pool,
  { compactWritesEnabled = false, compactWriter = createCompactMetricWriter(pool) } = {},
) {
  return {
    async upsertSleepSessions(sourceAccountId, sessions) {
      const client = await acquireDatabase(pool);
      try {
