...
import { createOxygenWriter } from './oxygen-writer.js';
...
export function createMetricWriter(
  ...
  return {
    ...createOxygenWriter(pool, { clientOwnedByCaller }),
    ...createSleepVitalsWriter(pool, { clientOwnedByCaller }),
      ...
      try {
        ...
        for (const session of sessions) {
          ...
          const written = await client.query(
            ...
            WHERE ${changedColumns('sleep_sessions', [
              'provider_id', 'civil_date', 'start_time', 'end_time', 'start_offset_seconds',
              'end_offset_seconds', 'sleep_type', 'is_nap', 'duration_seconds', 'asleep_seconds',
              'awake_seconds', 'efficiency', 'time_to_sleep_seconds', 'awake_episodes', 'device', 'source_fields',
            ])}
            RETURNING id`,
            [
              sessionId,
              ...
              session.minutesAsleep == null ? null : Math.round(Number(session.minutesAsleep) * 60),
              session.minutesAwake == null ? null : Math.round(Number(session.minutesAwake) * 60),
              session.efficiency ?? null,
              session.minutesToFallAsleep == null ? null : Math.round(Number(session.minutesToFallAsleep) * 60),
              Number(session.stageSummary?.awake?.count || 0),
              session.source?.device ?? {},
              session,
            ],
          );
          // The stored session includes its stages, so unchanged sessions need no stage writes.
      ...
      for (const summary of summaries) {
        await pool.query(
          ...
          ON CONFLICT (source_account_id, civil_date) DO UPDATE SET
            ...
            source_fields = EXCLUDED.source_fields,
            updated_at = CURRENT_TIMESTAMP
          WHERE ${changedColumns('heart_rate_daily_summaries', ['resting_bpm', 'resting_derived', 'source_fields'])}`,
          [
    ...
    async upsertHeartSamples(sourceAccountId, samples) {
      ...
      try {
        await client.query('BEGIN');
        await batchUpsert(client, 'heart_rate_samples', [
          ...
          sourceAccountId, sample.providerKey, sample.providerId ?? null, sample.civilDate,
          sample.sampledAt, sample.utcOffsetSeconds ?? null, sample.beatsPerMinute,
          sample.device ?? {}, sample.sourceFields ?? {},
        ]);
        await client.query('COMMIT');
      } catch (error) {
        await client.query('ROLLBACK');
      ...
      if (!compactWritesEnabled) return { inserted: 0, updated: 0, unchanged: 0, skipped: samples.length };
    ...
    async upsertCalorieIntervals(sourceAccountId, intervals) {
      ...
      try {
        await client.query('BEGIN');
        await batchUpsert(client, 'calorie_intervals', [
          ...
          sourceAccountId, interval.providerKey, interval.providerId ?? null, interval.civilDate,
          interval.metricType, interval.startTime, interval.endTime, interval.utcOffsetSeconds ?? null,
          interval.kilocalories, interval.device ?? {}, interval.sourceFields ?? {},
        ]);
        await client.query('COMMIT');
      } catch (error) {
        await client.query('ROLLBACK');
      ...
      if (!compactWritesEnabled) return { inserted: 0, updated: 0, unchanged: 0, skipped: intervals.length };
    ...
    async recalculateDaily(sourceAccountId, civilDate, database = pool) {
      ...
      const sleep = await database.query(
        `SELECT * FROM sleep_sessions
         ...
         ORDER BY duration_seconds DESC, id LIMIT 1`,
        [sourceAccountId, civilDate],
      );
