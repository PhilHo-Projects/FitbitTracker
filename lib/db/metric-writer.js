...
export function createMetricWriter(pool) {
  return {
    ...
    async recalculateDaily(sourceAccountId, civilDate) {
      const heartSamples = (
        await pool.query(
      ...
      if (heartCount || existingHeart) {
        const averageBpm = heartCount
          ? heartValues.reduce((sum, value) => sum + value, 0) / heartCount
          : null;
        await pool.query(
          `INSERT INTO heart_rate_daily_summaries (
          ...
          [
            ...
            averageBpm,
            heartCount ? Math.min(...heartValues) : null,
            heartCount ? Math.max(...heartValues) : null,
            heartCount,
      ...
      const [sleep, heart, calories] = await Promise.all([
        pool.query(
